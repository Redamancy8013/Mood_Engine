...
def audio_recorder_from_stream(url):
    global audio_buffer, segments_to_save, recording_active, last_active_time, last_vad_end_time

    url = url + "/audio_stream"
            ...
            for chunk in response.iter_content(chunk_size=CHUNK):
                ...
                if len(audio_buffer) * CHUNK / AUDIO_RATE >= 0.5:
                    ...
                    if check_vad_activity(raw_audio):
                        print("检测到语音活动")
                        last_active_time = time.time()
                        segments_to_save.append((raw_audio, last_active_time))
                    else:
                        print("静音中...")
                        pass
                    audio_buffer = []
                ...
                if time.time() - last_active_time > NO_SPEECH_THRESHOLD:
                    if segments_to_save:
                        last_segment = segments_to_save[-1]
                        if isinstance(last_segment, tuple) and len(last_segment) == 2:
                            last_segment_end_time = last_segment[1]
                            if last_segment_end_time > last_vad_end_time:
                                save_audio_video()
                                last_vad_end_time = last_segment_end_time
                                last_active_time = time.time()
...
def latest_audio():
    global audio_file_count, played_audio_files
    try:
        while True:
            ...
            time.sleep(0.1)
        # === 获取未播放过的音频文件列表 ===
        unplayed_files = [f for f in mp3_files if f not in played_audio_files]
        unplayed_files_with_time = [
            (f, os.path.getmtime(os.path.join(folder_path, f))) for f in unplayed_files
        ]
        latest_file = sorted(unplayed_files_with_time, key=lambda x: x[1])[-1][0]
        # 标记为已播放
        played_audio_files.add(latest_file)
        return jsonify({
...
if __name__ == "__main__":
    ...
    parser.add_argument('--url', type=str, default='https://d5b234f37b66.ngrok-free.app', help='情感流URL')
    parser.add_argument('--port', type=int, default=80, help='WebUI端口')
    parser.add_argument('--ngrok', action='store_true', help='使用ngrok进行端口映射')
    parser.add_argument('--push_url', type=str, default='https://ced359925868.ngrok-free.app', help='情感流URL')
